-- Clip 3.2 — Scalar and Table Macros
-- Demo file: m3_macros.sql
-- Dataset: data/users.csv, data/posts.csv


-- Query 1 — CREATE OR REPLACE MACRO (scalar)
...
-- Query 2 — Aggregate by tier using the macro
SELECT
    reputation_tier(CAST(Reputation AS INTEGER)) AS Tier,
    COUNT(*)                                      AS UserCount
FROM 'data/users.csv'
...
-- Query 3 — CREATE OR REPLACE MACRO (table macro)
